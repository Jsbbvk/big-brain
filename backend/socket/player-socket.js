...
const { getPlayer, setPlayerName } = require("../storage/player");
...
const init = (socket) => {
    ...
    socket.on("set player name", set_player_name);
};

const set_player_name = async (data, cb) => {
    let { room_id, player_uuid, name } = data;
    let res = await setPlayerName(player_uuid, name);
    cb && cb(res);

    emit(room_id, "player update name", Status({}, "success"));
};
...
const player_finish_voting = (socket) => async (data, cb) => {
    ...
    players.forEach((p, i) => {
        if (p === "skip_voting") {
            votedPlayers.skip =
                votedPlayers.skip == null ? 1 : votedPlayers.skip + 1;
        } else {
            votedPlayers[p.toString()] =
                votedPlayers[p.toString()] == null
                    ? 1
                    : votedPlayers[p.toString()] + 1;
        }
    });
    let maxFreq = 0;
    let max = [];
...
const continue_to_question = async (room_id) => {
    ...
    let { status: question_status } = await setRoundQuestion(
        room_id,
        difficulty
    );
...
const calculateRoundScore = async (roomId) => {
    ...
    if (!answers[0].length || !answers[1].length) {
        ...
        if (modifiers.includes(ModifierEnum["skip_deduction_increase"]))
            skips = 2;

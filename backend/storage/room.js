...
const getModifier = async (roomId, options) => {
    ...
    if (random_puuid !== undefined) {
        ...
        return Status(
            {
                modifier:
                    modifiers[getTrickster ? "trickster" : "samaritans"].enum,
            },
            "success"
        );
    } else if (player_uuid !== undefined) {
        if (
            modifiers.trickster.player_uuid.toString() ===
            ...
            let sam_mods = modifiers.samaritans.map((s) =>
                s.player_uuid.toString()
            );
...
const deleteRoomIfEmpty = async (roomId) => {
    ...
    if (room.players.length === 0) {
        let [, errDelete] = await handle(Room.deleteOne({ roomId }));
        if (errDelete) {
            console.log(err);
            return;
        }
